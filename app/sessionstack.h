...
class SessionStack : public QStackedWidget
{
	...
	public:
		...
		QJsonArray getSessionsAsJson(const QList<int> sessionIds);
		bool restoreSessionsFromJson(const QJsonArray &sessions);

		void setStartupCommandForTerminal(int terminalId, const QString &command);
		QString getStartupCommandForTerminal(int terminalId);


	public Q_SLOTS:
		int addSessionImpl(Session::SessionType type = Session::Single, const QJsonObject &json = QJsonObject());
